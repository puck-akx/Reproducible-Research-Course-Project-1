...
```{r, echo = TRUE}
setwd("C:/Users/Akanksha Singh/Documents/R")
...
##Step 6
Code to describe and show a strategy for imputing missing data using mean/mode value substitutions
...
There are 8 dates that have missing value. We don't exactly know the cause for these missing values but there's a pattern. For that matter, we can see that the mean value imputation is appropriate.

Every date has 288 data points. It means that the 8 dates have no data points at all what so ever. We can refine the analysis by looking at these missing values depending on their Weekday and interval parameters to matach with the average 
